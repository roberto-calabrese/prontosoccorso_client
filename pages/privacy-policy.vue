<template>
  <v-container>
    <v-row justify="center">
      <v-col cols="12" md="8">
        <v-card class="pa-4">
          <div class="text-center">
            <h1 class="text-h4 text-1">Privacy Policy</h1>
            <p class="text-1">Ultimo aggiornamento: <strong>09/09/2024</strong></p>
          </div>

          <v-card-text>
            <p>
              La presente Privacy Policy descrive come raccogliamo, utilizziamo e
              proteggiamo i dati relativi agli utenti del nostro sito web, in
              conformità con il Regolamento Generale sulla Protezione dei Dati (GDPR
              - Regolamento (UE) 2016/679). Il sito non raccoglie direttamente dati
              personali dagli utenti, salvo i dati tecnici necessari per il
              funzionamento e la sicurezza del sito.
            </p>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Titolare del Trattamento dei Dati</h3>
            <p>
              Il titolare del trattamento dei dati per questo sito web è:
            </p>
            <ul>
              <li><strong>Nome Cognome:</strong> Roberto Calabrese</li>
              <li><strong>Email di contatto:</strong> admin@prontosoccorso.live</li>
            </ul>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Tipi di Dati Raccolti</h3>
            <h4 class="mt-3 text-1">1. Dati raccolti automaticamente</h4>
            <p>
              Quando visiti il nostro sito web, i server registrano automaticamente
              alcune informazioni tecniche, come:
            </p>
            <ul>
              <li>Indirizzo IP</li>
              <li>Tipo di browser e sistema operativo</li>
              <li>Data e ora della visita</li>
              <li>Pagine visitate</li>
            </ul>
            <p>
              Questi dati sono necessari per il funzionamento e la sicurezza del sito
              web e vengono archiviati nei log dei server per finalità tecniche e di
              monitoraggio. Non vengono utilizzati per identificare gli utenti.
            </p>

            <h4 class="mt-3 text-1">2. Cookie e tecnologie di tracciamento</h4>
            <p>
              Utilizziamo tecnologie di tracciamento come i cookie per migliorare
              l'esperienza utente e garantire il funzionamento del sito. I cookie
              utilizzati si dividono in:
            </p>
            <ul>
              <li><strong>Cookie essenziali</strong>: necessari per il funzionamento del sito web, inclusi quelli di
                <strong>Pusher</strong>.
              </li>
              <li><strong>Cookie di analisi</strong>: per monitorare e analizzare il traffico sul sito web. Utilizziamo
                <strong>Google Analytics</strong> a tal fine, ma solo previo consenso dell'utente tramite il banner dei
                cookie.
              </li>
            </ul>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Uso dei Dati</h3>
            <p>I dati raccolti vengono utilizzati esclusivamente per:</p>

            <h4 class="mt-3 text-1">1. Funzionamento del sito (Pusher)</h4>
            <p>
              Utilizziamo <strong>Pusher</strong> per gestire la comunicazione in tempo reale sul sito. Questo è un
              servizio necessario per il corretto funzionamento del sito e non richiede consenso, in quanto rientra nei
              cookie essenziali.
            </p>

            <h4 class="mt-3 text-1">2. Monitoraggio e Statistiche (Google Analytics)</h4>
            <p>
              Utilizziamo <strong>Google Analytics</strong> per raccogliere informazioni anonime e statistiche sul
              traffico del sito, come l'indirizzo IP anonimizzato, comportamento di navigazione e altre metriche. Questi
              dati vengono raccolti solo se l'utente accetta i cookie tramite il banner.
            </p>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Base giuridica del trattamento</h3>
            <p>
              - <strong>Cookie essenziali (Pusher)</strong>: Il trattamento dei dati tecnici è necessario per il
              legittimo interesse del titolare del sito (art. 6(1)(f) del GDPR).
            </p>
            <p>
              - <strong>Cookie di analisi (Google Analytics)</strong>: Il trattamento dei dati di analisi si basa sul
              consenso esplicito dell'utente (art. 6(1)(a) del GDPR).
            </p>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Condivisione dei dati</h3>
            <p>
              I dati tecnici raccolti possono essere condivisi con i seguenti servizi:
            </p>
            <ul>
              <li>
                <strong>Pusher</strong>: per la gestione degli aggiornamenti in tempo reale. Maggiori dettagli sono
                disponibili nella loro <a href="https://bird.com/legal/privacy" target="_blank">Privacy
                Policy</a>.
              </li>
              <li>
                <strong>Google Analytics</strong>: per la raccolta di statistiche anonime, previa accettazione del
                consenso da parte dell'utente. Consulta la <a href="https://policies.google.com/privacy"
                                                              target="_blank">Privacy Policy di Google</a> per ulteriori
                informazioni.
              </li>
            </ul>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Conservazione dei dati</h3>
            <p>
              - <strong>Dati di log del server</strong>: Gli indirizzi IP e altre informazioni tecniche raccolte nei log
              del server vengono conservati per un periodo limitato.
            </p>
            <p>
              - <strong>Dati di Google Analytics</strong>: I dati statistici vengono conservati per il tempo necessario
              all'analisi, in conformità con le politiche di conservazione di Google.
            </p>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Diritti degli Utenti</h3>
            <p>
              Non raccogliendo dati personali direttamente dagli utenti, il sito limita il trattamento alle informazioni
              tecniche. Tuttavia, gli utenti hanno i seguenti diritti:
            </p>
            <ul>
              <li>Diritto di accesso ai propri dati (limitato ai dati tecnici raccolti)</li>
              <li>Diritto di rettifica (non applicabile, poiché non raccogliamo dati personali)</li>
              <li>Diritto alla cancellazione (solo per i dati tecnici)</li>
              <li>Diritto di opporsi al trattamento dei dati (applicabile in caso di tracciamento tramite Google
                Analytics)
              </li>
              <li>Diritto di presentare reclamo a un'autorità di controllo</li>
            </ul>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Modifiche a questa Privacy Policy</h3>
            <p>
              Ci riserviamo il diritto di modificare la presente Privacy Policy in qualsiasi momento. Le modifiche
              saranno pubblicate su questa pagina, con l'indicazione della data di aggiornamento.
            </p>

            <v-divider class="my-3"></v-divider>

            <h3 class="text-h6 mt-4 text-1">Contatti</h3>
            <p>
              Per qualsiasi domanda relativa alla presente Privacy Policy, puoi contattarci all'indirizzo email
              seguente:
            </p>
            <p><strong>Email:</strong> admin@prontosoccorso.live</p>

            <p class="mt-4">
              Questa Privacy Policy è stata aggiornata il: 09/09/2024.
            </p>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
</template>


<style scoped>
ul {
  margin-left: 20px;
}
</style>
<script setup lang="ts">
</script>